...
class Employee::DashboardControllerTest < ActionDispatch::IntegrationTest
  ...
  test "shows the signed in employees current clocking state" do
    ...
    assert_select ".today-week-lines dt", text: "Activitat", count: 0
    assert_select ".today-week-lines dt a[href='#{clockings_path(month: 7, year: 2026)}']", text: "Aquesta setmana"
    assert_select ".today-week-lines dd a[href='#{clockings_path(month: 7, year: 2026)}']", text: "0 h 00 min en 1 dia"
    assert_select ".today-week-lines dt a[href='#{clockings_path(month: 7, year: 2026)}']", text: "Aquest mes"
    assert_select ".today-week-lines dd a[href='#{clockings_path(month: 7, year: 2026)}']", text: "0 h 00 min"
  ...
  test "hides correction controls and previews when corrections are not allowed" do
    ...
    assert_response :success
    assert_select "a.today-correction-link", 0
    assert_select "a[href='#{corrections_path}']", 0
    assert_select ".employee-nav[style='--employee-nav-item-count: 3;']"
    assert_select ".today-week-lines dt", text: "Correccions", count: 0
    assert_select ".today-week-lines dt", text: "Aquesta setmana"
    assert_select ".today-week-lines dt", text: "Aquest mes"
    assert_select ".today-swipe-list .clocking-swipe.is-pending-requested", 0
    assert_equal [ "08:00" ], css_select(".today-swipe-list .clocking-swipe").map { |swipe| swipe.text.squish }
  end
